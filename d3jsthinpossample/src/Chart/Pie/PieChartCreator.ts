...
export class PieChartCreator{
    ...
    constructor(dataset: PieChartData[],
        size : Size,
        title: string,
        colors: string[],
        drawing : string){
    ...
    DrawChart(){
        ...
        // 2. SVG領域の設定
        var svg = d3.select(this.drawing).append("svg").attr("width", width).attr("height", height);
        ...
        let g = svg.append("g").attr("transform", "translate(" + width / 2 + "," + height / 2.5 + ")");
        
        // 3. カラーの設定
        var color = d3.scaleOrdinal().range(["#6080EA", "#01C89F", "#F28400"]);
        
        // 4. pieチャートデータセット用関数の設定
        var pie = d3.pie<PieChartData>()
            .value(function(d : PieChartData) { return d.value; })
            .sort(null);
        ...
        var legend = svg.selectAll('.legends')　// 凡例の領域作成
            .data(legendVals)
            .enter()
            .append('g')
            .attr("class", "legends")
